...
public class Camera2_Preview {
    ...
    private int getCameraDeviceIndex(int camIndex, int type, int direction) {
        ...
        try {
            ...
            for (int i = 0; i < camIds.length; ++i) {
                ...
                if (camInfoDirection >= 0 && camInfoDirection != (Integer)cc.get(CameraCharacteristics.LENS_FACING) || camIndex >= 0 && camIndex != i) continue;
                return i;
            }
        }
    ...
    private <T> CaptureRequest.Key<T> mapStringToKey(String keyString, CameraCharacteristics cc, T value) {
        List avilableKeys = cc.getAvailableCaptureRequestKeys();
    ...
    private boolean setCameraCaptureParams(CameraCacheInfo cci, int[] captureInfo, int[] overrideCaptureInfo) {
        ...
        StreamConfigurationMap scm = (StreamConfigurationMap)cci.characteristics.get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
        ...
        Size[] arr$ = sizes;
        int len$ = arr$.length;
        for (int i$ = 0; i$ < len$ && !(foundSize = (s = arr$[i$]).equals((Object)new Size(cci.requestWidth, cci.requestHeight))); ++i$) {
        }
        if (!foundSize) {
        ...
        Range[] fpsRanges = (Range[])cci.characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES);
        ...
        for (Range range : fpsRanges) {
            ...
            if (!range.contains((Comparable)Integer.valueOf(cci.requestFramerate)) || (size = (Integer)range.getUpper() - (Integer)range.getLower()) >= bestRangeSize) continue;
            bestRange = range;
            bestRangeSize = size;
        }
        if (bestRange == null) {
            DebugLog.LOGD("Camera2_Preview", String.format("setCameraCaptureParams: fps range not supported.", new Object[0]));
            return false;
        }
        cci.builder.set(CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE, (Object)bestRange);
        return true;
    }

    private boolean setupPreviewBuffer(CameraCacheInfo cci) {
        cci.reader = ImageReader.newInstance((int)cci.requestWidth, (int)cci.requestHeight, (int)cci.requestFormatAndroid, (int)2);
        ...
        cci.reader.setOnImageAvailableListener((ImageReader.OnImageAvailableListener)new OnCameraDataAvailable(), cci.handler);
    ...
    private List<Integer> getSupportedPreviewFrameRates(CameraCharacteristics cc) {
        Range[] frameRateRanges = (Range[])cc.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES);
        ...
        if (lowest < 0 || lowest >= 150 || highest < 0 || highest >= 300) {
            ...
            block1 : for (int i = lowest; i <= highest; ++i) {
                for (Range r2 : frameRateRanges) {
                    if (!r2.contains((Comparable)Integer.valueOf(i))) continue;
        ...
        if (SystemTools.checkMinimumApiLevel(21)) {
            try {
                ...
                if (cameraID < cameraIds.length) {
                    ...
                    return (Integer)cc.get(CameraCharacteristics.SENSOR_ORIENTATION);
                }
            }
    ...
    public int getDirection(int cameraID) {
        ...
        if (SystemTools.checkMinimumApiLevel(21)) {
            try {
                ...
                if (cameraID < cameraIds.length) {
                    CameraCharacteristics cc = this.mCameraManager.getCameraCharacteristics(cameraIds[cameraID]);
                    Integer lensFacing = (Integer)cc.get(CameraCharacteristics.LENS_FACING);
                    switch (lensFacing) {
                        case 1: {
                            return 268443665;
                        }
    ...
    public boolean close(int cameraCacheInfoIndex) {
        ...
        this.mCameraCacheInfoIndexCache.remove((Object)cci.reader);
    ...
    public int[] getCameraCapabilities(int cameraCacheInfoIndex) {
        ...
        try {
            ...
            scm = (StreamConfigurationMap)this.mCameraManager.getCameraCharacteristics(deviceId).get(CameraCharacteristics.SCALER_STREAM_CONFIGURATION_MAP);
        }
        ...
        if ((isFlashTorchSupported = (Boolean)cci.characteristics.get(CameraCharacteristics.FLASH_INFO_AVAILABLE)) == null) {
            isFlashTorchSupported = false;
        }
        boolean isFocusRegionSupported = (numSupportedFocusRegions = (Integer)cci.characteristics.get(CameraCharacteristics.CONTROL_MAX_REGIONS_AF)) != null && numSupportedFocusRegions > 0;
        Range aeCompensationRange = (Range)cci.characteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_RANGE);
        boolean isCompensationSupported = aeCompensationRange != null && !EMPTY_RANGE.equals((Object)aeCompensationRange);
    ...
    public int[] getCaptureInfo(int cameraCacheInfoIndex) {
        ...
        try {
            ...
            if (cci.builder != null) {
                Range fpsRange = (Range)cci.builder.get(CaptureRequest.CONTROL_AE_TARGET_FPS_RANGE);
    ...
    public boolean start(int cameraCacheInfoIndex) {
        ...
        try {
            ...
            cci.session.setRepeatingRequest(cci.builder.build(), (CameraCaptureSession.CaptureCallback)new OnFrameCapturedCallback(cci), cci.handler);
    ...
    Object getUntypedCameraParameter(int cameraCacheInfoIndex, String name) {
        ...
        for (int i2 = 0; i2 < cameraCharacteristicsKeys.size(); ++i2) {
            CameraCharacteristics.Key key = (CameraCharacteristics.Key)cameraCharacteristicsKeys.get(i2);
            if (!key.getName().equals(name)) continue;
            ...
            try {
                block3 : switch (type) {
                    case 538968064: {
                        ...
                        cci.builder.set(CaptureRequest.CONTROL_CAPTURE_INTENT, (Object)(val != 0 ? 3 : 1));
                        break;
                    }
                    case 536870913: {
                        ...
                        switch (torchMode) {
                            case 805306370: {
                                cci.builder.set(CaptureRequest.FLASH_MODE, (Object)2);
                                break block3;
                            }
                            case 805306369: {
                                cci.builder.set(CaptureRequest.FLASH_MODE, (Object)0);
                                break block3;
                            }
                            case 805306372: {
                    ...
                    case 536870914: {
                        ...
                        switch (focusMode) {
                            ...
                            case 805306400: {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AF_MODE, (Object)1);
                            ...
                            case 805306432: {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AF_MODE, (Object)3);
                                break block3;
                            }
                            case 805306496: {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AF_MODE, (Object)2);
                                doPostSetAction = true;
                                break block3;
                            }
                            case 805306624: {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AF_MODE, (Object)0);
                                cci.builder.set(CaptureRequest.LENS_FOCUS_DISTANCE, (Object)Float.valueOf(0.0f));
                                break block3;
                            }
                            case 805306880: {
                                if (Arrays.binarySearch(supportedFocusModes, 0) == -1) {
                    ...
                    case 536870916: {
                        ...
                        cci.builder.set(CaptureRequest.LENS_FOCUS_DISTANCE, (Object)focusDist);
                        break;
                    }
                    case 536870920: {
                        SystemTools.setSystemErrorCode(6);
                    ...
                    case 536870928: {
                        ...
                        Integer numRegons = (Integer)cci.characteristics.get(CameraCharacteristics.CONTROL_MAX_REGIONS_AF);
                        ...
                        cci.builder.set(CaptureRequest.CONTROL_AF_REGIONS, (Object)focusRects);
                        doPostSetAction = true;
                        break;
                    }
                    case 536870944: {
                        int exposureMode = ((Number)value).intValue();
                        int[] aeModes = (int[])cci.characteristics.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_MODES);
                        ...
                        switch (exposureMode) {
                            case 805310464: {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AE_MODE, (Object)0);
                                break block3;
                            }
                            case 805314560: 
                            case 805322752: {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AE_MODE, (Object)1);
                                break block3;
                            }
                        }
                        SystemTools.setSystemErrorCode(3);
                    ...
                    case 536870976: 
                    case 537919488: {
                        Range range = (Range)cci.characteristics.get(CameraCharacteristics.SENSOR_INFO_SENSITIVITY_RANGE);
                        if (range == null || CaptureRequest.SENSOR_SENSITIVITY == null) {
                            SystemTools.setSystemErrorCode(6);
                            return false;
                        }
                        int _value = ((Number)value).intValue();
                        if (range.contains((Comparable)Integer.valueOf(_value))) {
                            cci.builder.set(CaptureRequest.SENSOR_SENSITIVITY, (Object)_value);
                            break;
                        }
                        break;
                    }
                    ...
                    case 536871168: {
                        ...
                        Range expCompRange = (Range)cci.characteristics.get(CameraCharacteristics.CONTROL_AE_COMPENSATION_RANGE);
                        if (EMPTY_RANGE.equals((Object)expCompRange) || CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION == null) {
                            SystemTools.setSystemErrorCode(6);
                            return false;
                        }
                        if (expCompRange.contains((Comparable)Integer.valueOf(expCompValue))) {
                            cci.builder.set(CaptureRequest.CONTROL_AE_EXPOSURE_COMPENSATION, (Object)expCompValue);
                            break;
                        }
                        break;
                    }
                    ...
                    case 536871936: {
                        ...
                        switch (whiteBalanceMode) {
                            case 805371904: {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AWB_LOCK, (Object)true);
                                break block3;
                            }
                            case 805437440: 
                            case 805568512: {
                                if (CaptureRequest.CONTROL_AWB_LOCK != null && ((Boolean)cci.builder.get(CaptureRequest.CONTROL_AWB_LOCK)).booleanValue()) {
                                    cci.builder.set(CaptureRequest.CONTROL_AWB_LOCK, (Object)false);
                                }
                    ...
                    case 536879104: {
                        ...
                        float[] zoomValues = (float[])cci.characteristics.get(CameraCharacteristics.LENS_INFO_AVAILABLE_FOCAL_LENGTHS);
                        ...
                        for (float possibleZoomValue : zoomValues) {
                            ...
                            setIt = true;
                            cci.builder.set(CaptureRequest.LENS_FOCAL_LENGTH, (Object)Float.valueOf(possibleZoomValue));
                            break;
                        }
                        if (!setIt) {
                            SystemTools.setSystemErrorCode(2);
                    ...
                    case 545259520: {
                        ...
                        cci.builder.set(CaptureRequest.CONTROL_VIDEO_STABILIZATION_MODE, (Object)(videoStabilizationMode ? 1 : 0));
                        break;
                    }
                    default: {
                        return false;
                ...
                try {
                    switch (type) {
                        case 536870914: {
                            ...
                            if (maxRegions != null && CameraCharacteristics.CONTROL_MAX_REGIONS_AF != null && maxRegions > 0 && CaptureRequest.CONTROL_AF_REGIONS != null && (regions = (MeteringRectangle[])cci.builder.get(CaptureRequest.CONTROL_AF_REGIONS)) != null && regions.length > 0) {
                                ...
                                cci.builder.set(CaptureRequest.CONTROL_AF_REGIONS, (Object)newRegions);
                            }
                        }
                        case 536870928: {
    ...
    Object getTypedCameraParameter(int cameraCacheInfoIndex, int type) {
        ...
        try {
            switch (type) {
                ...
                case 536870920: {
                    Pair focusRange = (Pair)captureResult.get(CaptureResult.LENS_FOCUS_RANGE);
    ...
    extends CameraCaptureSession.CaptureCallback {
        ...
        public void onCaptureCompleted(CameraCaptureSession session, CaptureRequest request, TotalCaptureResult result) {
            ...
            if (request.equals((Object)this.mCancelRequest) && afState == 0) {
                try {
                    this.mCCI.session.capture(this.mFocusRequest, (CameraCaptureSession.CaptureCallback)this, this.mCCI.handler);
                }
                catch (CameraAccessException var5_5) {}
            } else if (request.equals((Object)this.mFocusRequest) && (afState == 4 || afState == 5)) {
    ...
    implements ImageReader.OnImageAvailableListener {
        ...
        public void onImageAvailable(ImageReader reader) {
            ...
            Integer index = (Integer)Camera2_Preview.this.mCameraCacheInfoIndexCache.get((Object)reader);
            if (index == null) {
                DebugLog.LOGE("Camera2_Preview", "Unable to find reader in the index cache!");
                Trace.endSection();
                return;
            }
            CameraCacheInfo _cci = (CameraCacheInfo)Camera2_Preview.this.mCameraCacheInfos.get(index);
            if (_cci == null) {
